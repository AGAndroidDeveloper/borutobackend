...
class HeroRepoImpl() : HeroRepository {
    override val heros: Map<Int, List<Hero>>
        get() = mapOf(
            1 to page1,
            2 to page2,
            3 to page2,
    ...
    override suspend fun getAllHero(page: Int): ApiResponse {
        return ApiResponse(
            true, "sending paginated  data", heroes = heros[page]!!,
            prevPage = returnPreviousKey(page = page),
            nextPage = returnNextKey(page = page)
        )
    }

    override suspend fun getSearchHero(name: String): ApiResponse {
        return getSearchero(name)
    }

    override suspend fun getAllHeroList(): ApiResponse {
        return ApiResponse(
            success = true,
            message = "sending all hero",
            heroes = heros.values.flatten(),
            prevPage = null,
            nextPage = null,
            lastUpdated = null
        )
    ...
    private fun getSearchero(name: String): ApiResponse {
        val search = name.toLowerCasePreservingASCIIRules()
        val mhero = mutableListOf<Hero>()
        ...
        if (heros.isNotEmpty()) {
            heros.forEach { (key, value) ->
                heros[key]?.forEach() { hero ->
                    if (hero.name.toLowerCasePreservingASCIIRules() == search) {
        ...
        println(mhero.toString())
        return ApiResponse(success = true, heroes = mhero)

    }


    private fun returnPreviousKey(page: Int): Int? {
        return if (page == 1) {
            null
        } else {
            page - 1
    ...
    private fun returnNextKey(page: Int): Int? {
        return if (page == heros.size) {
            null
        } else {
            page + 1
        }
